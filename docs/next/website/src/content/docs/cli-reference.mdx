...
herdr pane report-agent-session <pane_id> \
  ...
  [--task-id ID --spawn-kind KIND --generation N [--parent-task-id ID]]
...
`report-agent-session` updates native session identity without reporting lifecycle state. It accepts the same optional conceptual task provenance as `agent start`; Herdr applies it only when it accepts the reported session identity, and omitting it leaves existing provenance unchanged. `release-agent` ends that source's lifecycle authority when its agent process exits.
...
herdr agent start <name> --kind KIND --pane ID [--timeout MS] [--task-id ID --spawn-kind KIND --generation N [--parent-task-id ID]] [-- <agent-args...>]
...
Conceptual task provenance is optional. Supply `--task-id`, `--spawn-kind`, and `--generation` together; add `--parent-task-id` for a child task or omit it for a root task. Existing starts without these flags remain unchanged. `agent get` returns the stored identity at `.result.agent.task_provenance`, so callers can use its `task_id` as a later start's parent.
