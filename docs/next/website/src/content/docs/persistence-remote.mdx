...
Saved destinations also live in the app: open the global menu and pick
`connections` to switch between the local server and saved SSH destinations, or
add one with `+ ssh`. Choosing a different destination detaches the client so it
comes back attached there; the panes on the machine you leave keep running.
`herdr connection list|add|use|rename|remove` does the same from a shell.
